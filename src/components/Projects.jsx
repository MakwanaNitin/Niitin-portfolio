...
const MISSIONS = [
  {
    id: '01',
    name: 'MU Lost & Found',
    description: 'A Flutter-based mobile application for Marwadi University that helps students report, search, and recover lost and found items through a simple and user-friendly interface.',
  ...
  },
  {
    id: '02',
    name: 'Chat-X',
    description: 'A desktop chatting application featuring secure authentication, customizable themes, database integration, dynamic IP handling, and real-time communication.',
  ...
  },
  {
    ...
    description: 'A web application designed to help students explore career opportunities using AI-powered guidance and recommendations.',
...
const ProjectCard = ({ project }) => {
  ...
  return (
    ...
    >
      ...
      <div 
        ref={glowRef}
        className="absolute w-64 h-64 bg-yellow-500/10 rounded-full blur-[80px] pointer-events-none z-10 -translate-x-1/2 -translate-y-1/2 opacity-0 mix-blend-screen"
      />
      
      {/* Laser Border overlay on Hover */}
      <div className="absolute inset-0 border border-yellow-500/0 group-hover:border-yellow-500/40 rounded-xl transition-colors duration-500 z-20 pointer-events-none" />

      {/* Foreground Content */}
      <div className="absolute inset-0 z-30 p-8 flex flex-col justify-end pointer-events-none" style={{ transform: 'translateZ(30px)' }}>
        
        {/* Dossier Code */}
        <div className="text-yellow-400/80 font-mono text-xs tracking-[0.3em] mb-2 font-bold uppercase transition-transform duration-500 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100">
          File_ID // {project.id}
        </div>
        ...
        <div className="mt-auto pointer-events-auto w-fit">
          <a href={project.link} className="flex items-center space-x-2 text-sm uppercase tracking-[0.2em] font-medium text-white group/link relative">
            <span className="relative overflow-hidden block">
              ...
              <span className="block absolute inset-0 translate-y-full group-hover/link:translate-y-0 transition-transform duration-300 text-yellow-400">Access Record</span>
            </span>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="transform group-hover/link:translate-x-1 group-hover/link:-translate-y-1 transition-transform duration-300 text-yellow-400">
        ...
        <div ref={headerRef} className="mb-16 md:mb-24 flex flex-col md:flex-row md:items-end justify-between border-b border-white/10 pb-8">
          <div className="max-w-2xl">
            <h2 className="text-4xl md:text-5xl lg:text-[4.5rem] font-bold tracking-tighter text-white font-sans leading-none drop-shadow-lg mb-4">
              Declassified <span className="font-serif italic font-light opacity-80 text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-white pr-2">Missions</span>
            </h2>
          </div>
